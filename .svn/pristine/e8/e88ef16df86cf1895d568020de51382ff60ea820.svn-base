<?php 
    ...
    define("ADMIN_LOGIN_ENTITY", "AdminUser"); // set this to the correct entity.
	 
	class AdminLogin extends PLController 
	{ 
		public $errorMessage;
		
		public function __construct($formData) 
		{ 
			parent::__construct($formData, "AdminLogin");
		...
		{
			$this->goToLocation("AdminDashboard");
		}
		...
		public function validateLogin()
		{
			...
			$quals = array(
                ...
                new BLKeyValueQualifier("password", OP_EQUAL, $enc_password),
            );
            $result = BLGenericRecord::find(ADMIN_LOGIN_ENTITY, new BLAndQualifier($quals), null, array("limit" => 1));
		...
		public function sendPasswordReminder()
        {
            ...
            $user = BLGenericRecord::recordMatchingKeyAndValue(ADMIN_LOGIN_ENTITY, "email", $emailStr);
            if ($user)
            {
                $email = new PLEmail();
